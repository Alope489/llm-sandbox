"""Generate plots and a report from a multi-file-growth benchmark run.

Reads ``metrics_averaged.csv`` (and optionally ``metrics_per_query.csv``)
from a timestamped run directory produced by
``benchmark_multi_file_growth.py`` and writes PNGs and ``report.md``
...
Usage::
    ...
    python new_kb_sandbox/plot_multi_file_growth.py --results-dir new_kb_sandbox/results/multi_file/2026-04-06T120000
...
_TEST_LABEL = "Multi-File Growth (Test 2)"
...
def generate_artifacts(results_dir: Path) -> dict[str, Path]:
    """Load CSVs and produce all benchmark artifacts for a multi-file run.
    ...
    Args:
        results_dir: Timestamped benchmark run directory (e.g.
            ``new_kb_sandbox/results/multi_file/2026-04-06T120000``).
...
def _parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate plots and report for Test 2 (multi-file growth) benchmark.",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(e.g. new_kb_sandbox/results/multi_file/2026-04-06T120000)."
        ),
    )
...
def main() -> None:
    """Entry point: generate all artifacts for a multi-file-growth run.
